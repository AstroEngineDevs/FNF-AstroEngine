...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create() {
		...
		var menuBG:FlxSprite = new FlxSprite().loadGraphic(backend.utils.Paths.image('menuBGBlue'));
		menuBG.setGraphicSize(Std.int(menuBG.width * 1.1));
		menuBG.updateHitbox();
		menuBG.screenCenter();
		menuBG.antialiasing = backend.utils.ClientPrefs.data.globalAntialiasing;
		add(menuBG);
		...
		for (i in 0...options.length) {
			...
			var optionText:game.objects.Alphabet = new game.objects.Alphabet(280, 300, Achievements.isAchievementUnlocked(achieveName) ? Achievements.achievementsStuff[achievementIndex[i]][0] : '?', false);
