...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create() {
		...
		menuBG.updateHitbox();
		menuBG.screenCenter();
		menuBG.color = EngineData.coreGame.menuColor;
		menuBG.antialiasing = funkin.backend.utils.ClientPrefs.data.globalAntialiasing;
		add(menuBG);
		...
		for (i in 0...options.length) {
			...
			var optionText:funkin.game.objects.Alphabet = new funkin.game.objects.Alphabet(280, 300, Achievements.isAchievementUnlocked(achieveName) ? Achievements.achievementsStuff[achievementIndex[i]][0] : '?', false);
