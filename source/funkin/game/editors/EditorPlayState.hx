...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (notetype in noteTypeMap.keys()) {
			...
			if(sys.FileSystem.exists(luaToLoad)) {
				var lua:funkin.game.editors.EditorLua = new funkin.game.editors.EditorLua(luaToLoad);
		...
		scoreTxt.visible = !ClientPrefs.data.hideHud || !ClientPrefs.data.hideFullHUD;
		add(scoreTxt);
			...
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if(daStrumTime >= startPos) {
						...
						if(!Std.isOfType(songNotes[3], String)) swagNote.noteType = funkin.game.editors.ChartingState.noteTypeList[songNotes[3]]; //Backward compatibility + compatibility with Week 7 charts
	...
	private function endSong() {
		LoadingState.loadAndSwitchState(new funkin.game.editors.ChartingState());
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image('$pixelShitPart1/rating/' + daRating));
		rating.screenCenter();
		...
		rating.visible = !ClientPrefs.data.hideHud || !ClientPrefs.data.hideFullHUD;
		...
		comboSpr.visible = !ClientPrefs.data.hideHud || !ClientPrefs.data.hideFullHUD;
		...
		for (i in seperatedScore)
		{
			...
			numScore.visible = !ClientPrefs.data.hideHud || !ClientPrefs.data.hideFullHUD;
