...
class ChartingState extends MusicBeatState
{
	...
	var strumLineNotes:FlxTypedGroup<game.objects.StrumNote>;
	...
	**/
	...
	override function create()
	{
		...
		// backend.utils.Paths.clearMemory();
		...
		var eventIcon:FlxSprite = new FlxSprite(-GRID_SIZE - 5, -90).loadGraphic(backend.utils.Paths.image('eventArrow'));
		...
		currentSongName = backend.utils.Paths.formatToSongPath(_song.song);
		loadSong();
		reloadGridLayer();
		...
		for (i in 0...8){
			var note:game.objects.StrumNote = new game.objects.StrumNote(GRID_SIZE * (i+1), strumLine.y, i % 4, 0);
		...
		for (i in 0...tipTextArray.length) {
			...
			tipText.y += i * 12;
			tipText.setFormat(backend.utils.Paths.font("vcr.ttf"), 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		{
			currentSongName = backend.utils.Paths.formatToSongPath(UI_songTitle.text);
			loadSong();
			updateWaveform();
		});
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			var songName:String = backend.utils.Paths.formatToSongPath(_song.song);
			var file:String = backend.utils.Paths.json(songName + '/events');
			#if sys
			if (#if MODS_ALLOWED FileSystem.exists(backend.utils.Paths.modsJson(songName + '/events')) || #end FileSystem.exists(file))
			#else
		...
		#if MODS_ALLOWED
		var directories:Array<String> = [backend.utils.Paths.mods('characters/'), backend.utils.Paths.mods(backend.utils.Paths.currentModDirectory + '/characters/'), backend.utils.Paths.getPreloadPath('characters/')];
		for(mod in backend.utils.Paths.getGlobalMods())
			directories.push(backend.utils.Paths.mods(mod + '/characters/'));
		#else
		var directories:Array<String> = [backend.utils.Paths.getPreloadPath('characters/')];
		#end

		var tempMap:Map<String, Bool> = new Map<String, Bool>();
		var characters:Array<String> = CoolUtil.coolTextFile(backend.utils.Paths.txt('characterList'));
		...
		#if MODS_ALLOWED
		var directories:Array<String> = [backend.utils.Paths.mods('stages/'), backend.utils.Paths.mods(backend.utils.Paths.currentModDirectory + '/stages/'), backend.utils.Paths.getPreloadPath('stages/')];
		for(mod in backend.utils.Paths.getGlobalMods())
			directories.push(backend.utils.Paths.mods(mod + '/stages/'));
		#else
		var directories:Array<String> = [backend.utils.Paths.getPreloadPath('stages/')];
		#end

		tempMap.clear();
		var stageFile:Array<String> = CoolUtil.coolTextFile(backend.utils.Paths.txt('stageList'));
	...
	function addNoteUI():Void
	{
		...
		#if MODS_ALLOWED
		directories.push(backend.utils.Paths.mods('custom_notetypes/'));
		directories.push(backend.utils.Paths.mods(backend.utils.Paths.currentModDirectory + '/custom_notetypes/'));
		for(mod in backend.utils.Paths.getGlobalMods())
			directories.push(backend.utils.Paths.mods(mod + '/custom_notetypes/'));
		#end
	...
	function addEventsUI():Void
	{
		...
		#if MODS_ALLOWED
		directories.push(backend.utils.Paths.mods('custom_events/'));
		directories.push(backend.utils.Paths.mods(backend.utils.Paths.currentModDirectory + '/custom_events/'));
		for(mod in backend.utils.Paths.getGlobalMods())
			directories.push(backend.utils.Paths.mods(mod + '/custom_events/'));
		#end
	...
	function generateSong() {
		FlxG.sound.playMusic(backend.utils.Paths.inst(currentSongName), 0.6/*, false*/);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				LoadingState.loadAndSwitchState(new game.editors.EditorPlayState(sectionStartTime()));
			}
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							FlxG.sound.play(backend.utils.Paths.sound(soundToPlay)).pan = note.noteData < 4? -0.3 : 0.3; //would be coolio
		...
		if(metronome.checked && lastConductorPos != Conductor.songPosition) {
			...
			if(metroStep != lastMetroStep) {
				FlxG.sound.play(backend.utils.Paths.sound('Metronome_Tick'));
				//trace('Ticked');
			}
		}
	...
	function loadAudioBuffer() {
		...
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(backend.utils.Paths.modFolders('songs/' + currentSongName + '/Inst.ogg'))) {
			audioBuffers[0] = AudioBuffer.fromFile(backend.utils.Paths.modFolders('songs/' + currentSongName + '/Inst.ogg'));
			//trace('Custom vocals found');
		}
		else { #end
			var leVocals:String = backend.utils.Paths.getPath(currentSongName + '/Inst.' + backend.utils.Paths.SOUND_EXT, SOUND, 'songs');
		...
		audioBuffers[1] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(backend.utils.Paths.modFolders('songs/' + currentSongName + '/Voices.ogg'))) {
			audioBuffers[1] = AudioBuffer.fromFile(backend.utils.Paths.modFolders('songs/' + currentSongName + '/Voices.ogg'));
			//trace('Custom vocals found');
		} else { #end
			var leVocals:String = backend.utils.Paths.getPath(currentSongName + '/Voices.' + backend.utils.Paths.SOUND_EXT, SOUND, 'songs');
	...
	*/
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		#end
		{
			path = backend.utils.Paths.getPreloadPath('characters/' + Character.DEFAULT_CHARACTER + '.json'); //If a character couldn't be found, change him to BF just to prevent a crash
		}
		...
		var json:game.objects.characters.Character.CharacterFile = cast Json.parse(rawJson);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(backend.utils.Paths.font("vcr.ttf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(daSus != null) { //Common note
			...
			note.loadGraphic(backend.utils.Paths.image('eventArrow'));
	...
	private function saveLevel()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), backend.utils.Paths.formatToSongPath(_song.song) + ".json");
		}
	}
