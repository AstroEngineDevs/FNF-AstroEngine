...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		{
			game.states.StoryMenuState.weekCompleted = FlxG.save.data.weekCompleted;
		}
		...
		#else
		if(FlxG.save.data.flashing == null && !game.states.FlashingState.leftState) {
			FlxTransitionableState.skipNextTransIn = true;
			FlxTransitionableState.skipNextTransOut = true;
			MusicBeatState.switchState(new game.states.FlashingState());
		} else {
			if (initialized)
				startIntro();
			else
	...
	function startIntro()
	{
		...
		if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.length > 0 && titleJSON.backgroundSprite != "none"){
			bg.loadGraphic(backend.utils.Paths.image(titleJSON.backgroundSprite));
		}else{
		...
		switch(easterEgg.toUpperCase())
		{
			#if TITLE_SCREEN_EASTER_EGG
			case 'SHADOW':
				gfDance.frames = backend.utils.Paths.getSparrowAtlas('ShadowBump');
			...
			case 'RIVER':
				gfDance.frames = backend.utils.Paths.getSparrowAtlas('RiverBump');
			...
			case 'SHUBS':
				gfDance.frames = backend.utils.Paths.getSparrowAtlas('ShubBump');
			...
			case 'BBPANZU':
				gfDance.frames = backend.utils.Paths.getSparrowAtlas('BBBump');
		...
		var path = "mods/" + backend.utils.Paths.currentModDirectory + "/images/titleEnter.png";
		//trace(path, FileSystem.exists(path));
		...
		titleText.frames = backend.utils.Paths.getSparrowAtlas('titleEnter');
		#end
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(backend.utils.Paths.image('newgrounds_logo'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	{
		var fullText:String = Assets.getText(backend.utils.Paths.txt('introText'));
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				{
					if (mustUpdate) {
						MusicBeatState.switchState(new game.states.OutdatedState());
					} else {
						MusicBeatState.switchState(new game.states.MainMenuState());
					}
					closedState = true;
				});
				// FlxG.sound.play(backend.utils.Paths.music('titleShoot'), 0.7);
			}
			#if TITLE_SCREEN_EASTER_EGG
				...
				if(allowedKeys.contains(keyName)) {
					...
					for (wordRaw in easterEggKeys)
					{
						...
						if (easterEggKeysBuffer.contains(word))
						{
							...
							FlxG.sound.play(backend.utils.Paths.sound('ToggleJingle'));
							...
							{
								game.states.FreeplayState.vocals.fadeOut();
							}
							closedState = true;
							transitioning = true;
							playJingle = true;
	...
	{ 
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				{
					case 'RIVER':
						sound = FlxG.sound.play(backend.utils.Paths.sound('JingleRiver'));
					case 'SHUBS':
						sound = FlxG.sound.play(backend.utils.Paths.sound('JingleShubs'));
					case 'SHADOW':
						FlxG.sound.play(backend.utils.Paths.sound('JingleShadow'));
					case 'BBPANZU':
						sound = FlxG.sound.play(backend.utils.Paths.sound('JingleBB'));
