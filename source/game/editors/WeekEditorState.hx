...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		var ui_tex = backend.utils.Paths.getSparrowAtlas('campaign_menu_UI_assets');
		var bgYellow:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 386, 0xFFF9CF51);
		bgSprite = new FlxSprite(0, 56);
		bgSprite.antialiasing = backend.utils.ClientPrefs.data.globalAntialiasing;

		weekThing = new MenuItem(0, bgSprite.y + 396, weekFileName);
		weekThing.y += weekThing.height + 20;
		weekThing.antialiasing = backend.utils.ClientPrefs.data.globalAntialiasing;
		add(weekThing);
		...
		lock.antialiasing = backend.utils.ClientPrefs.data.globalAntialiasing;
		add(lock);
		
		missingFileText = new FlxText(0, 0, FlxG.width, "");
		missingFileText.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y + 435).loadGraphic(backend.utils.Paths.image('Menu_Tracks'));
		tracksSprite.antialiasing = backend.utils.ClientPrefs.data.globalAntialiasing;
		add(tracksSprite);

		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 60, 0, "", 32);
		txtTracklist.alignment = CENTER;
		txtTracklist.font = backend.utils.Paths.font("vcr.ttf");
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(backend.utils.Paths.modsImages('menubackgrounds/menu_' + assetName)) || #end
			Assets.exists(backend.utils.Paths.getPath('images/menubackgrounds/menu_' + assetName + '.png', IMAGE), IMAGE)) {
				bgSprite.loadGraphic(backend.utils.Paths.image('menubackgrounds/menu_' + assetName));
				isMissing = false;
			}
		}
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(backend.utils.Paths.modsImages('storymenu/' + assetName)) || #end
			Assets.exists(backend.utils.Paths.getPath('images/storymenu/' + assetName + '.png', IMAGE), IMAGE)) {
				weekThing.loadGraphic(backend.utils.Paths.image('storymenu/' + assetName));
				isMissing = false;
			}
		}
...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	function changeSelection(change:Int = 0) {
		FlxG.sound.play(backend.utils.Paths.sound('scrollMenu'), 0.4);
