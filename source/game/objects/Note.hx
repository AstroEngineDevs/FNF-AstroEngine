...
class Note extends FlxSprite
{
	...
	function reloadNote(?prefix:String = '', ?texture:String = '', ?suffix:String = '') {
		...
		if(PlayState.isPixelStage) {
			if(isSustainNote) {
				loadGraphic(backend.utils.Paths.image('pixelUI/' + blahblah + 'ENDS'));
				width = width / 4;
				height = height / 2;
				originalHeightForCalcs = height;
				loadGraphic(backend.utils.Paths.image('pixelUI/' + blahblah + 'ENDS'), true, Math.floor(width), Math.floor(height));
			} else {
				loadGraphic(backend.utils.Paths.image('pixelUI/' + blahblah));
				width = width / 4;
				height = height / 5;
				loadGraphic(backend.utils.Paths.image('pixelUI/' + blahblah), true, Math.floor(width), Math.floor(height));
			}
