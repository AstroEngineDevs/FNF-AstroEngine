...
class CharacterEditorState extends MusicBeatState
{
	...
	{
		//FlxG.sound.playMusic(backend.utils.Paths.music('breakfast'), 0.5);
		...
		healthBarBG = new FlxSprite(30, FlxG.height - 75).loadGraphic(backend.utils.Paths.image('healthBar'));
		...
		textAnim.setFormat(backend.utils.Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function addCharacterUI() {
		...
		noAntialiasingCheckBox.callback = function() {
			...
			if(!noAntialiasingCheckBox.checked && backend.utils.ClientPrefs.data.globalAntialiasing) {
	...
	function reloadCharacterImage() {
		...
		if(backend.utils.Paths.fileExists('images/' + char.imageFile + '/Animation.json', TEXT)) {
			char.frames = AtlasFrameMaker.construct(char.imageFile);
		} else if(backend.utils.Paths.fileExists('images/' + char.imageFile + '.txt', TEXT)) {
			char.frames = backend.utils.Paths.getPackerAtlas(char.imageFile);
		} else {
			char.frames = backend.utils.Paths.getSparrowAtlas(char.imageFile);
		}
	...
	function reloadCharacterDropDown() {
		...
		#else
		characterList = CoolUtil.coolTextFile(backend.utils.Paths.txt('characterList'));
		#end
