...
class StrumNote extends FlxSprite
{
	...
	public function new(x:Float, y:Float, leData:Int, player:Int)
	{
		...
		var skin:String = NoteUtils.CURRENTSKIN;
	...
	public function reloadNote()
	{
		...
		{
			loadGraphic(Paths.image(NoteUtils.getNotePath(PIXEL) + texture));
			width = width / 4;
			height = height / 5;
			loadGraphic(Paths.image(NoteUtils.getNotePath(PIXEL) + texture), true, Math.floor(width), Math.floor(height));
		...
		{
			frames = Paths.getSparrowAtlas(NoteUtils.getNotePath(NORMAL) + texture);
