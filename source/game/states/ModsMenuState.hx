...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt.setFormat(backend.utils.Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(FileSystem.exists(path))
		{
			...
			for (i in 0...leMods.length)
			{
				if(leMods.length > 1 && leMods[0].length > 0) {
					...
					if(!backend.utils.Paths.ignoreModFolders.contains(modSplit[0].toLowerCase()))
					{
		...
		if (FileSystem.exists("modsList.txt")){
			for (folder in backend.utils.Paths.getModDirectories())
			{
				if(!backend.utils.Paths.ignoreModFolders.contains(folder))
				{
		...
		buttonToggle = new FlxButton(startX, 0, "ON", function()
		{
			...
			updateButtonToggle();
			FlxG.sound.play(backend.utils.Paths.sound('scrollMenu'), 0.6);
		});
		...
		buttonToggle.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER);
		...
		buttonUp.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonDown.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonTop.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonDisableAll.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		buttonEnableAll.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 24, FlxColor.BLACK, CENTER);
		...
		installButton.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER);
		...
		{
			var path = haxe.io.Path.join([backend.utils.Paths.mods(), modsList[curSelected][0]]);
		...
		removeButton.label.setFormat(backend.utils.Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER);
		...
		descriptionTxt.setFormat(backend.utils.Paths.font("vcr.ttf"), 32, FlxColor.WHITE, LEFT);
		...
		while (i < modsList.length)
		{
			...
			if(!FileSystem.exists(backend.utils.Paths.mods(values[0])))
			{
			...
			var iconToUse:String = backend.utils.Paths.mods(values[0] + '/pack.png');
			...
			{
				newMod.icon.loadGraphic(backend.utils.Paths.image('unknownMod'));
			}
	...
	override function update(elapsed:Float)
	{
		...
		if(canExit && controls.BACK)
		{
			...
			if(needaReset)
			{
				...
				game.states.TitleState.initialized = false;
				game.states.TitleState.closedState = false;
				FlxG.sound.music.fadeOut(0.3);
				if(game.states.FreeplayState.vocals != null)
				{
					game.states.FreeplayState.vocals.fadeOut(0.3);
					game.states.FreeplayState.vocals = null;
				}
...
class ModMetadata
{
	...
	public function new(folder:String)
	{
		...
		this.color = game.states.ModsMenuState.defaultColor;
		this.restart = false;

		//Try loading json
		var path = backend.utils.Paths.mods(folder + '/pack.json');
		if(FileSystem.exists(path)) {
