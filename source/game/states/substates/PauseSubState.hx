...
class PauseSubState extends MusicBeatSubstate
{
	...
	var menuItemsOG:Array<String> = ['Resume', 'Restart Song', 'Change Difficulty', 'Exit', 'Exit to menu'];
	...
	var skipTimeTracker:game.objects.Alphabet;
	...
	public function new(x:Float, y:Float)
	{
		...
		levelInfo.setFormat(backend.utils.Paths.font("vcr.ttf"), 32);
		...
		levelDifficulty.setFormat(backend.utils.Paths.font('vcr.ttf'), 32);
		...
		blueballedTxt.setFormat(backend.utils.Paths.font('vcr.ttf'), 32);
		blueballedTxt.updateHitbox();
		add(blueballedTxt);

		practiceText = new FlxText(20, 15 + 101, 0, "PRACTICE MODE", 32);
		practiceText.scrollFactor.set();
		practiceText.setFormat(backend.utils.Paths.font('vcr.ttf'), 32);
		...
		chartingText.setFormat(backend.utils.Paths.font('vcr.ttf'), 32);
		...
		grpMenuShit = new FlxTypedGroup<game.objects.Alphabet>();
		add(grpMenuShit);
	...
	function regenMenu():Void {
		...
		for (i in 0...menuItems.length) {
			var item = new game.objects.Alphabet(90, 320, menuItems[i], true);
			...
			if(menuItems[i] == 'Skip Time')
			{
				...
				skipTimeText.setFormat(backend.utils.Paths.font("vcr.ttf"), 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
