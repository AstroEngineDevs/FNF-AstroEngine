...
class NotesSubState extends MusicBeatSubstate
{
	...
	private var grpNumbers:FlxTypedGroup<funkin.game.objects.Alphabet>;
	...
	var hsbText:funkin.game.objects.Alphabet;
	...
	public function new() {
		...
		add(grpNotes);
		grpNumbers = new FlxTypedGroup<funkin.game.objects.Alphabet>();
		add(grpNumbers);

		for (i in 0...ClientPrefs.data.arrowHSV.length) {
			var yPos:Float = (165 * i) + 35;
			for (j in 0...3) {
				var optionText:funkin.game.objects.Alphabet = new funkin.game.objects.Alphabet(posX + (225 * j) + 250, yPos + 60, Std.string(ClientPrefs.data.arrowHSV[i][j]), true);
				grpNumbers.add(optionText);
			}

			var note:FlxSprite = new FlxSprite(posX, yPos);
			note.frames = Paths.getSparrowAtlas(NoteUtils.getNotePath(NORMAL) + 'NOTE_assets');
		...
		hsbText = new funkin.game.objects.Alphabet(posX + 560, 0, "Hue    Saturation  Brightness", false);
		hsbText.scaleX = 0.6;
		hsbText.scaleY = 0.6;
		add(hsbText);
