...
class FreeplayState extends MusicBeatState
{
	...
	private var grpSongs:FlxTypedGroup<funkin.game.objects.Alphabet>;
	...
	override function create()
	{
		...
		grpSongs = new FlxTypedGroup<funkin.game.objects.Alphabet>();
		add(grpSongs);

		for (i in 0...songs.length)
		{
			var songText:funkin.game.objects.Alphabet = new funkin.game.objects.Alphabet(90, 320, songs[i].songName, true);
	...
	//		icon.animation.curAnim.curFrame = 1;
		...
		var swag:funkin.game.objects.Alphabet = new funkin.game.objects.Alphabet(1, 0, "swag");
	...
	{
		songs.push(new SongMetadata(songName.formatText(), weekNum, songCharacter, color));
	}

	function weekIsLocked(name:String):Bool {
		var leWeek:WeekData = WeekData.weeksLoaded.get(name);
		return (!leWeek.startUnlocked && leWeek.weekBefore.length > 0 && (!funkin.game.states.StoryMenuState.weekCompleted.exists(leWeek.weekBefore) || !funkin.game.states.StoryMenuState.weekCompleted.get(leWeek.weekBefore)));
	}
