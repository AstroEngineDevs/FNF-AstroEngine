...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(FileSystem.exists(path))
		{
			...
			for (i in 0...leMods.length)
			{
				if(leMods.length > 1 && leMods[0].length > 0) {
					...
					if(!backend.utils.Paths.ignoreModFolders.contains(modSplit[0].toLowerCase()) && !modsAdded.contains(modSplit[0]))
					{
						if(modSplit[1] == '1')
		...
		var arrayOfFolders:Array<String> = backend.utils.Paths.getModDirectories();
		arrayOfFolders.push('');
		...
		var list:Array<Array<String>> = [ //Name - Icon name - Description - Link - BG Color
			['Astro Engine Team'],
			['Hackx2',		'hackx2',		'Main Programmer of Astro Engine',								'https://twitter.com/AnimatesHack',	'BA7EFF'],
			[''],
			['Psych Engine Team'],
		...
		for (i in 0...creditsStuff.length)
		{
			...
			var optionText:game.objects.Alphabet = new game.objects.Alphabet(FlxG.width / 2, 300, creditsStuff[i][0], !isSelectable);
			...
			if(isSelectable) {
				...
				{
					backend.utils.Paths.currentModDirectory = creditsStuff[i][5];
				}
				...
				if(creditsStuff[i][1] == "face")
					str = 'credits/nullObj';
				if (backend.utils.Paths.image('credits/' + creditsStuff[i][1]) != null)
	...
	function pushModCreditsToList(folder:String)
	{
		...
		if(folder != null && folder.trim().length > 0) creditsFile = backend.utils.Paths.mods(folder + '/data/credits.txt');
		else creditsFile = backend.utils.Paths.mods('data/credits.txt');
