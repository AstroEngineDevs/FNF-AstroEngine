...
class StrumNote extends FlxSprite
{
	...
	public function reloadNote()
	{
		...
		{
			loadGraphic(backend.utils.Paths.image('pixelUI/' + texture));
			width = width / 4;
			height = height / 5;
			loadGraphic(backend.utils.Paths.image('pixelUI/' + texture), true, Math.floor(width), Math.floor(height));
		...
		{
			frames = backend.utils.Paths.getSparrowAtlas(texture);
